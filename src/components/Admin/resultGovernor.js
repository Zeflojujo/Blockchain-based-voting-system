...
import governorContractAbi from "../../abi/CandidateGovernor.json";
...
const ResultBlockLeader = (props) => {
  ...
  const handleRegisterGovernor = async (event) => {
    try{
      ...
      const abi = governorContractAbi.abi;
      const address = governorContractAbi.networks[5777].address;
      ...
      await contract.methods.registerGovernor(firstName, lastName, college, program, yearOfStudy, regNo, blockNumber, vAddress).send({ from: selectedAccount, gas: 1000000 });
