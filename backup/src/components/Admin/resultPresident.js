...
// import contractAbi from "../../abi/CandidatePresident.json";
...
const ResultPresident = (props) => {
  ...
  const handleClose = () => {
    ...
    const handleRegisterPresident = async (event) => {
      try{
        ...
        await contract.methods.registerPresident(firstName, lastName, college, program, yearOfStudy, regNo, blockNumber, gender).send({ from: selectedAccount, gas: 1000000 });
        ...
        if(error.message.includes("Only admin can perform this action")){
          ...
          setErrorMessage(errorMessage);
          setErrorMessageOpen(true);
        }else if (error.message.includes("The registered president is enough!")) {
          const errorMessage = "Error Error! The Registered president is enough!!!"
          setErrorMessage(errorMessage);
          setErrorMessageOpen(true);
        } else {
          setErrorMessage("");
    ...
    const formik = useFormik({
      ...
      onSubmit: handleRegisterPresident,
    });
    ...
    const rows = Presidents.map((block, index) => ({
      id: index + 1,
                ...
                Registered Presidents
                  ...
                  <Button
                        ...
                        sx={{
                          ...
                          width: isXs || isSm ? "140px": isMd ? "200px": "250px",
                          display: 'flex',
                          flexDirection: 'column',
                          alignItems: 'center',
                        }}
